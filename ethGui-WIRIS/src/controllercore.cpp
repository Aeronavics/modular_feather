...
void ControllerCore::paintCenter(bool paint){
    _center->show = paint;
}
...
void ControllerCore::toggleStreamThermal(){
    ...
    if (!_streamFlagThermal)
    {
        ...
        _thermalThread = new ThermalThread(&this->_streamFlagThermal, this->_endpointIp, _maximum, _center, _minimum, &_paintVals, zoom, &_cameraType);
...
void ControllerCore::setCurrentPaletteByIndex(int i){
    ...
    {
        qDebug() << _palettes.at(i)->value().toStdString().c_str();
...
void ControllerCore::toggleHelperThread()
{
    ...
    if (!_helperTreadFlag)
    {
        ...
        _helperThread = new HelperThread(&_helperTreadFlag, &_extremesFlag, fp, _maximum, _center, _minimum, &_firmwareVersion, &_cameraType);
...
void ControllerCore::fetchPalettes(){
    ...
    if (send("GPTL", iss))
    {
        while(getAnyLine(iss, cache))
        {
            if(!cache.empty())
            {
                this->_palettes.push_back(new PaletteWrapper(QString::fromStdString(cache).split(" ")[1]));
            }
        }
